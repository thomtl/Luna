...
namespace xhci {
    ...
    struct HCI {
        ...
        std::unique_ptr<Promise<bool>> send_ep_bulk(Port& port, uint8_t epid, std::span<uint8_t> data);
        std::unique_ptr<Promise<bool>> send_ep_irq(Port& port, uint8_t epid, std::span<uint8_t> data);
